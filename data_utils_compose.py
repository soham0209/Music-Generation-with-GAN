# Author Konstantin
# Modified by Soham Mukherjee,Pavani Komati and Rupendra Nath Mitra with permission from Author for CSE 5523: Project. Cited in project report.
...
def NetOutToPianoRoll(network_output, threshold=0.1):
    ...
    for i, timestep in enumerate(network_output):
        if np.amax(timestep) > threshold:
            pos = 0
            pos = int(np.argmax(timestep[:]) / timestep.shape[0])
...
def createMidiFromPianoRoll(piano_roll, lowest_note, directory, mel_test_file, threshold, res_factor=1):
    ...
    for j in range(piano_roll.shape[0]-1):  # all values between first and last one
        ...
        for k in range(piano_roll.shape[1]):
            if (np.all(piano_roll[j+1, k] == 1) and np.all(piano_roll[j, k] == 0)) or (np.all(piano_roll[j+1, k] == 0) and np.all(piano_roll[j, k] == 1)):  # only do something if note_on or note_off are to be set
                if set_note == 0:
                ...
                if np.all(piano_roll[j+1, k]) and np.all(piano_roll[j, k] == 0):
                    set_note += 1
                    track.append(Message('note_on', note=k+lowest_note, velocity=100, time=time))
                if np.all(piano_roll[j+1, k] == 0) and np.all(piano_roll[j, k] == 1):
                    set_note += 1
